...
```{r echo=TRUE, paged.print=TRUE, layout="l-body-outset", eval=FALSE}
...
rm(sec1ents, dat, text, url1, sec1)
```
...
I turned off the previous code chunks, so that the batch processing can run on a clean slate. Now let's get this sucker running in batch mode on several files. The setup is that we need a text file in the working directory with a list of URLs to scrape. We did this earlier in the script when we loaded in `sections.csv` as the variable `sections`. 
